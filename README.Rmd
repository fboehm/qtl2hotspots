...
The goal of hotspots is to analyze the Keller et al. (2018) expression trait hotspots.
...
To render the manuscript `paper.Rnw`, into pdf, use a shell to enter the following text from the 'hotspots' directory:

```{bash, docker_build, eval = FALSE}
docker build .
```

If you're not using Docker, and have cloned the repository from Github, you can use the Makefile to render `paper.Rnw`. Just type `make` in the shell (from within the `hotspots` directory):
