---
title: "Heatmaps of pleiotropy test statistics - all traits above LOD 10"
...
library(hotspots)
pleio_threshold <- 4
ult <- 10 # univariate lod threshold
```

Below, we produce heatmaps for every hotspot. We consider, for every hotspot, all local and nonlocal traits with univariate LOD at least `r ult`. Note that the choice of `r ult` is arbitrary and limits the sets of traits to those that are rather strongly associated.

In future analyses, we might subset only the local traits... ie, use only the strongest local traits and all of the nonlocals
...
```{r tabulate-2, message=FALSE, warning = FALSE, eval = FALSE}
med_tib <- med_list %>%
  purrr::map(bind_rows) %>%
  bind_rows() %>%
  as_tibble() %>%
