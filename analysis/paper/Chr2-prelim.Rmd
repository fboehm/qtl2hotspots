...
```{r 2x2_table_proportion}
table(med = foo$lod_diff_proportion > 0.4, pleio = foo$pleiotropy_lod <= pleiotropy_threshold)
```
...
foo %>% 
  filter(lod_diff_proportion > 0.2, pleiotropy_lod <= pleiotropy_threshold) %>%
  select(local_symbol, nonlocal_symbol, pleiotropy_lod, lod_diff_proportion, lod_diff)
  arrange(desc(nonlocal_symbol)) 
```
