...
foohc <- foo %>%
  ...
  (function(x) {
    hc <- dist(x) %>% hclust()
    return(x[hc$order, hc$order])
    }
   ) %>%
  (function(x) {
    cols <- c("green", "orange")[1 + (colnames(x) %in% bar$nonlocal_symbol)]
    heatmap.2(x, symm = TRUE, dendrogram = "both", na.rm = TRUE, ColSideColors = cols, cexRow = 0.2, cexCol = 0.2)
              }
   )
```
