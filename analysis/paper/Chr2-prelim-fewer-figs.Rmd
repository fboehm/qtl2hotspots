...
pleio_results_mat %>%
  ...
  hclust() %>% 
  rafalib::myplclust(
                     lab.col = 1 + (rownames(pleio_results_mat) %in% local_annot3$annot.id), cex = 0.1
                     )
```
...
## Clustering of the rectangular nlocal by nnonlocal matrix
...
med_thresh <- 2
foomat <- foo %>%
  select(1, 2, 4) %>% # choose lod_diff_proportion as stat of interest
  tibble_to_matrix()
...
pleio_resd0 <- apply(FUN = function(x){x[is.na(x)] <- 0; return(x)}, X = pleio_results_mat, MARGIN = 2)
