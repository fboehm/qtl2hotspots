...
statistical computing environment \citep{r}.
...
<<readRDS-2>>=
pleio_mixed_list <- readRDS(here("analysis", "data", paste0("Chr", hot_chr, "_pleio.rds")))
pleio_local_list <- readRDS(here("analysis", "data", paste0("Chr", hot_chr, "_pleio_local.rds")))
pleio_nonlocal_list <- readRDS(here("analysis", "data", paste0("Chr", hot_chr, "_pleio_nonlocal.rds")))
med_list <- readRDS(here("analysis", "data", paste0("Chr", hot_chr, "_mediations.rds")))
@
...
for (i in 1:n_figs){
  ...
  ggsave(filename = file.path("../figures", fnsvg), height = ceiling(n_panels / 2))
  ggsave(filename = file.path("../figures", fnpdf), height = ceiling(n_panels / 2))
}
@
...
<<heat9-2, fig.height = 7, fig.width = 7, out.width = "\\textwidth">>=
...
p <- pm[ord, ord] %>%
  ...
  hide_guides()
fnsvg <- paste0("Chr", hot_chr, "_heat", ".svg")
fnpdf <- paste0("Chr", hot_chr, "_heat", ".pdf")
plotly::orca(p, file.path("../figures", fnsvg))
plotly::orca(p, file.path("../figures", fnpdf))
p
@
