...
We generated scatter plots of pleiotropy test statistics vs. mediation LOD difference proportion values. Each point represents a pairing of a local trait with a nonlocal trait. In all five scatter plots, we observe few points in the upper right quadrant of the figure. That is, few pairs have high pleiotropy test statistics and high mediation LOD difference values.
...
an answer about the number of underlying QTL. A drawback of pleiotropy testing is that the scientific question that it addresses - do two traits share a single QTL - informs, but doesn't fully resolve, the genetics of the nonlocal complex trait under study.
