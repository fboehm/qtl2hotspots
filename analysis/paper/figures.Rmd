...
for (i in 1:n_figs){
  ...
  foo %>%
    ...
    ggplot() + 
      ...
      ylab("Mediation LOD difference") + 
      geom_text(vjust = 0.9, hjust = 0.9, size = 3, aes(x = max_pleio_lod / 2, y = 2 * max_med_lod_difference / 3, label = local_symbol), inherit.aes=FALSE)
  fnsvg <- paste0("Chr", hot_chr, "_scatter-panel", i, ".svg")
  fnpdf <- paste0("Chr", hot_chr, "_scatter-panel", i, ".pdf")
  ggsave(filename = file.path("../figures", fnsvg), height = 9)
  ggsave(filename = file.path("../figures", fnpdf), height = 9)
}
```
