...
nonlocal transcripts at each of the five hotspots. We defined a "local trait" to be one whose corresponding gene lay within the hotspot. We focused on those local 
traits that strongly associated with QTL within the hotspot. To limit the scope of our analyses, 
we only studied those expression traits with univariate LOD greater than 15.
...
with a QTL in the hotspot. Additionally, we required that the nonlocal trait's gene not lie on the hotspot's chromosome. We ignored transcripts for genes on the hotspot chromosome 
...
One hierarchical clustering involved only the rows containing local traits, while the other contained only 
