...
### Allele effects contrasts

```{r, define-submap-{{hot_chr}}}
figdir <- here::here("analysis", "figures", "Chr{{hot_chr}}-allele-effects-plots")
names(s1coef_out) -> ids # need to run inline code above to define s1coef_out
# find indices on Chr5 for boundaries of hotspot
which(map[[hot_chr]] > hot_lower)[1] -> start_index
which(map[[hot_chr]] > hot_upper)[1] -> stop_index
submap <- list()
submap$`5` <- map[[hot_chr]][start_index:stop_index]
mylist <- list()
for (i in 1:(nlocal + nnonlocal)) {
...
purrr::pmap(.l = list(s1coef_out, as.list(ids), mylist), 
           .f = function(x, y, z){
             main <- local_nonlocal_annot %>%
               ...
               paste0()
             fn <- here::here("analysis", "figures", "Chr{{hot_chr}}-allele-effects", paste0(main, ".pdf"))
             pdf(file = fn)
...
```{r, barplots-scan1coef-effects-{{hot_chr}}, fig.height = 20, fig.width = 8}
...
ggsave(here::here("analysis", "figures", "allele_effects_at_peaks-{{hot_chr}}.png"), limitsize = FALSE)
```
