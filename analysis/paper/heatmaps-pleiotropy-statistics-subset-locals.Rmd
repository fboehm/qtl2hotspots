...
```{r heat-pleio-2}
heatmaply(pm, symm = TRUE, cexRow = 0.2, cexCol = 0.2, row_side_colors = scols, limits = c(0,4))
```
...
pm %>%
  ...
  (function(x) {rownames(x) <- colnames(x); return(x)}) %>%
  heatmaply(cexRow = 0.2, cexCol = 0.2, symm = TRUE, row_side_colors = scols, limits = c(0, 5))
